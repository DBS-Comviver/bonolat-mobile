...
export function useFractioningContext(boxCode?: string, boxItems: any[] = []): UseFractioningContextReturn {
	...
	useEffect(() => {
		if (boxCode && cod_estabel && cod_deposito) {
			loadLocations();
		} else {
			setLocations([]);
		}
	...
	const loadDeposits = async () => {
		...
		try {
			const itemCode = boxItems.length > 0 ? boxItems[0].it_codigo : "GENERIC";
			const data = await fractioningApi.getDeposits(cod_estabel, itemCode);
